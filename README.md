...
使用状态机管理 NPC 的行为状态。支持枚举状态管理、黑板数据共享和状态转移。
...
## 车辆NPC
- Idle和Move状态和人类NPC一样。新增了Dead状态。
- 需要注意的是碰撞检测类的功能需要在Controller中，通过参数传给黑板实现。因为状态类不继承`Monobehaviour`。

### Dead状态(CarDeadState)
- 进入该状态时直接Destroy拥有该状态的实例
